...
El alcance de este proyecto comprende el diseño e implementación de un sistema inteligente de visión por computador para el seguimiento automático de nadadores, compuesto por una **cámara fija** cuya calibración se realiza a partir de las **medidas físicas estáticas predefinidas de la piscina** (longitud de carril, marcas de fondo, banderas de vuelta). El video capturado es procesado mediante un módulo de **preprocesamiento y estandarización de frames**, cuya salida alimenta un modelo **YOLO con tracking integrado** encargado de la detección y el seguimiento de los nadadores. Esta lógica de procesamiento se ejecuta en un **Servidor de IA**, que se comunica de forma sincronizada con un **Servidor Web** encargado de validar, persistir y exponer la información en una **base de datos**. El acceso a la plataforma se realiza a través de una **página web** con **autenticación por roles** (administrador, entrenador, nadador), desde la cual los usuarios pueden visualizar **estadísticas en tiempo real**, consultar el **historial de entrenamientos** y generar **reportes exportables en PDF y CSV** con métricas de tiempo por vuelta, tiempo total, distancia recorrida, velocidad promedio, ritmo de nado y tiempos de descanso.
